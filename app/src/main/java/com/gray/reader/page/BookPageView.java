...
public class BookPageView extends WriteView implements IReaderPage {
    ...
    private int defaultWidth = UIUtils.getDisplayWidth(getContext());//默认宽度
    private int defaultHeight = UIUtils.getDisplayHeight(getContext());//默认高度

    private int viewWidth = UIUtils.getDisplayWidth(getContext());
    private int viewHeight = UIUtils.getDisplayHeight(getContext());

    float lPathAShadowDis = 5;//A区域左阴影矩形短边长度参考值
    float rPathAShadowDis = 5;//A区域右阴影矩形短边长度参考值
    ...
    private ArrayList<Element> secondPage;
    ...
    private void init(Context context) {
        ...
        pathCPaint.setColor(Color.WHITE);
        pathCPaint.setAntiAlias(true);//设置抗锯齿

        pathCContentPaint = new Paint();
        pathCContentPaint.setColor(Color.WHITE);
    ...
    private void drawPathAContentBitmap(Bitmap bitmap, Paint pathPaint) {
        ...
        //下面开始绘制区域内的内容...
        pathPaint.setColor(pageProperty.bgColor);
        resetPaint(textPaint);
        mCanvas.drawPath(getPathDefault(), pathPaint);
//        mCanvas.drawText("这是在A区域的内容...AAAA", viewWidth - 260, viewHeight - 100, paint);
//        mCanvas.drawColor(0xffFDDFE7, PorterDuff.Mode.CLEAR);
        if (page == null) {
            return;
        }
        for (Element element : page) {
            ...
            element.draw(mCanvas, textPaint);
            resetPaint(textPaint);
        }
        //结束绘制区域内的内容...
    }
    ...
    private void drawPathBContentBitmap(Bitmap bitmap, Paint pathPaint) {
        ...
        resetPaint(textPaint);
        mCanvas.drawPath(getPathDefault(), pathPaint);
//        mCanvas.drawText("这是在B区域的内容...BBBB", viewWidth - 260, viewHeight - 100, paint);
//        mCanvas.drawColor(0xffFDDFE7, PorterDuff.Mode.CLEAR);
        if (secondPage == null) {
            return;
        }
        for (Element element : secondPage) {
            if (element instanceof BottomElement) {
                BottomElement element1 = (BottomElement) element;
                element1.setIndex(index + "/" + count);
                element1.setPower(power);
...
//        chooseStyle(moveX, moveY);
//        setTouchPoint(moveX, moveY, style);
    }
...
//        chooseStyle(moveX, moveY);
//        setTouchPoint(defaultWidth, defaultHeight, style);
    }
...
//        setTouchPoint(defaultWidth, defaultHeight, style);
    ...
    public ValueAnimator animCurrentToPrevious(float changeX, final float moveY, float moveX) {
        chooseStyle(moveX, moveY);
        Log.e("animCurrentToPrevious", "animCurrentToPrevious: ");
        ValueAnimator animator = ValueAnimator.ofFloat(defaultWidth, ~defaultWidth / 3);
        animator.setDuration(500);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float curValue = (float) animation.getAnimatedValue();
//                setTouchPoint(curValue, curValue, style);
            }
        });
        return animator;
    }
    ...
    public ValueAnimator animCurrentToNext(float changeX, final float moveY, float moveX) {
        ...
        ValueAnimator animator = ValueAnimator
                .ofFloat(moveX, UIUtils.getDisplayWidth(getContext()));
        Log.e("animCurrentToNext", "animCurrentToNext: ");
        animator.setDuration(500);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
...
//        setY(0f);
    ...
    public void reset() {
        setDefaultPath();
        chooseStyle(defaultWidth, defaultHeight);
//        setTouchPoint(defaultWidth, defaultHeight, style);
    }

    private void chooseStyle(float x, float y) {
        if (x <= viewWidth / 3) {//左
            style = STYLE_LEFT;
    ...
    public void setSecondPage(ArrayList<Element> page) {
        secondPage = page;
    }
